#!/usr/bin/env bash
# 맥(iTerm2) 또는 윈도우 Git Bash/WSL에서 실행.
# 실행 전: cp .env.example .env 로 복사 후 CONTEXT7_API_KEY 값을 채우고
#         `set -a; source .env; set +a` 로 불러온 뒤 이 스크립트를 실행하세요.
set -e

: "${CONTEXT7_API_KEY:?CONTEXT7_API_KEY 환경변수를 먼저 설정하세요 (.env.example 참고)}"
...
echo "완료. 'claude mcp list' 로 확인하세요. supabase는 첫 사용 시 브라우저 OAuth 로그인이 뜰 수 있습니다."
